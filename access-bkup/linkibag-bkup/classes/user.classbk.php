...
class userClass extends commonClass
{
	...
	function user_reset_password($username){
		...
		if(isset($row['uid']) and $row['uid']>0){
			...
			if(isset($row['email_id'])){
				...
				$message = 'Dear<br /><br /><p>Hello, You are requesting for password at Linkibag. <br /> Your password : <b>'.$row['pass'].'</b> <br/>You can update your password by clicking this link or copying and pasting it to your browser:</p> '.$verified_link.'<br /><br />Cheers<br />Team Linkibag';				
				$from = 'info@mwsdemo.com';				
	...
	function create_user($postdata, $files){
		...
		$postdata['verified_link'] = WEB_ROOT.'/verify.php?user='.$user_id.'&v='.$verify_code;			
		$mail_content = $this->mail_format('new_register', $postdata);
		$this->send_email($to, $mail_content['subject'], $mail_content['matter']);			
		$this->setmessage("status", "Login detail has been sent to your given email address.");			
		return $user_id;
