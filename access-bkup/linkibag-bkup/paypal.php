...
switch ($_GET['action']) {
	...
	case 'success':      // Order was successful...
		...
		if(isset($payment['subs_id']) and $payment['subs_id'] > 0){
			...
			unset($new_val);
			$co->query_update('user_subscriptions', array('subs_payment_id'=>$pay_id,'subs_status'=>1), array('id'=>$payment['subs_id']), 'subs_id=:id');
			
			//change current role
			$co->query_update('users', array('role'=>$payment['subs_package']), array('id'=>$current['uid']), 'uid=:id');
