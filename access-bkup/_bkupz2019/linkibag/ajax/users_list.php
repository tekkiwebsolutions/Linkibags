...
if (!empty($_POST) ) {
	$table_column_name = array('u.uid', 'p.first_name', 'p.last_name', 'u.email_id', 'u.role', 'u.status', 'u.created', 'u.last_login_time', 'u.uid');
    ...
    /* END SEARCH */
        ...
        $sql = "select u.uid, p.first_name, p.last_name, u.email_id, u.role, u.status, u.created, u.last_login_time from users u, profile p where u.uid=p.uid ORDER BY ".$orderBy." ".$orderType." limit ".$start.", ".$length;
		$data = $co->fetch_all_array($sql,array());
