...
all you have to do is click the link below:'.
	...
	function get_paging_link($total_pages, $this_page){
		...
		$page_link = '<a class="btn btn-default dark-gray-bg" href="#">Go to page</a>                 
					...
					<button data-toggle="dropdown" type="button" class="btn btn-default dropdown-toggle" aria-expanded="false">'.((isset($_GET['page']) and $_GET['page'] != '') ? $_GET['page'] : '1').' <li class="caret"></li></button>
	...
	function list_all_friends_of_current_user($uid, $status, $item_per_page, $this_page,$fgroup){	
		...
		LEFT JOIN users u ON ur.fid=u.uid WHERE ur.uid=:id";
		$cond = array();
		$cond['id'] = $uid;
		if($fgroup != "no"){
	...
	function get_share_link_popup($url_id){	
		...
		$share_link_body = '		
			...
			function users_count_url($uid){	
				...
				$row = $this->query_first($sql, array('id2'=>$uid,'id'=>$uid));		
			...
			function users_count_friend($uid){
				...
				$sql = "SELECT COUNT(friend_id) as total_friend FROM `user_friends` WHERE uid=:id and status='1'";		
