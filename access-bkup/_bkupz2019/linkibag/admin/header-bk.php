    ...
    <div id="wrapper">
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            ...
            <div class="collapse navbar-collapse navbar-ex1-collapse">
				...
				if(isset($_GET['p'])){
					if(strpos($_GET['p'], 'user_management') !== false){
						$user_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'url_management') !== false){
						$ad_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'page_management') !== false){
						$page_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'admin_url_management') !== false){
						$farmtalk_tags_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'topic_management') !== false){
						$topic_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'category_management') !== false){
						$banner_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'state') !== false){
						$state_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'city') !== false){
						$city_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'faq') !== false){
						$faq_nav_link = 'on';
					}
					if(strpos($_GET['p'], 'blog') !== false){
						$blog_nav_link = 'on';
					}
                ...
                <ul class="nav navbar-nav side-nav">
					...
					<li>
                        ...
                        <ul id="banner_manage_link" class="collapse demo<?=($banner_nav_link=='on' ? ' in' : '')?>">							
							...
							<li>
								<a href=" main.php?p=category_management/manage">Manage default category</a>
							</li> 
							<li>
								<a href=" main.php?p=category_management/manage_user_category">Manage user category</a>
							</li> 
                        </ul>
                    </li>
					<li>
