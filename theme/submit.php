		...
		*/
		...
		if(isset($_POST['email_id']) and $_POST['email_id']!=''){
			...
			if(filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
				$msg .= "Please use user@gmail.com format to register with LinkiBag <br/>";
				$success=false;
				...
				if(isset($emails_ids) and count($emails_ids) > 5){
					...
					foreach($emails_ids as $email_ids){
						...
						if(isset($already_send_request['total']) and $already_send_request['total'] >= 3){
							$co->setmessage("error", "sorry you crossed the maxmimum send 3 request to ".$email_ids);
							$success=false;
							break;
						}
  ...
  ';
					...
					if(isset($result['ids']) and $result['ids'] > 0){
						if(isset($_POST['type']) and  $_POST['type'] == 'category'){
							...
							if(isset($_POST['group_delete']) and  $_POST['group_delete'] == 1){
								...
								$msg_content = 'User was removed successfully.';	
	...
	if(isset($_POST['form_id']) and $_POST['form_id']=="register_other_country"){								
		...
		if(isset($_POST['email_id']) and $_POST['email_id']!=''){
			...
			$domain_name_array = array('@hotmail.com', '@yahoo.com', '@gmail.com', '@Hotmail.com', '@Yahoo.com', '@Gmail.com');
			if(filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
				$msg .= "Please use user@gmail.com format to complete your process <br/>";
				$success=false;
			}elseif(!in_array($domain_name, $domain_name_array)){
				$msg .= "LinkiBag did not recognize your email vendor as a public email provider from our list <br/>";
				$success=false;
			}/*elseif($co->is_emailExists($email)){
				$msg .= "The email address you have entered is already registered.  <br/>";
				$success=false;
			}*/
		}
