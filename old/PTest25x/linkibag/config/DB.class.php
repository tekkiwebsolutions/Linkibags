...
class DB
{
       ...
       */        
        ...
        */
		...
		///
		///
		function fetch_all_array($sql,$data)
		{
			return $row=$this->query($sql,$data);
		}
		//
		// Onely single row values
		function query_first($sql,$data)
		{
		...
		function query_insert($table, $data)
		{
			...
			$k = implode(",",$keys);
			$k_v = implode(",",$key_value);
			if($this->query("INSERT INTO `$table`($k) VALUES($k_v)",$data))
			{
		...
		///
		///
		function query_update($table, $data, $where_data, $where)
		{
			foreach($data as $key=>$val)
			{
				$keys[]=$key." = :".$key;
			...
			$this->query("UPDATE `$table` SET $k WHERE $where",$data);
		}
		///
		///
		function query_delete($table,$data,$where=null)
		{
			if($this->query("DELETE FROM `$table` WHERE ".$where,$data))
			{
				return true;
			}
			else
