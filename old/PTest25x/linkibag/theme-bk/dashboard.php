      ...
      <div class="container">
         ...
         <div class="tab-content">
            <form name="dash_form" method="post" id="share_urls_from_dash" action="index.php?p=dashboard&ajax=ajax_submit">
                  ...
                  <div style="margin-bottom: 11px;" class="user-name-dash">
                     <div class="row">
                        ...
                        <?php } ?>
                           ...
                           <a class="btn button-grey pull-right" href="javascript: void(0);" onclick="multiple_load_share_link_form('share');"><i class="fa fa-share-alt" aria-hidden="true"></i> Share</a>
                        </div>
                        ...
                        <table class="border_block table table-design margin-none" id="all_records">
                           <tbody>
                              ...
                              <tr class="<?=$class_name.$show_friend_class?><?=$urlpost['num_of_visits'] > 0 ? ' read' : ' unread'?>" id="url_<?=$urlpost['shared_url_id']?>">
                                 ...
                                 <td style="width:28%; text-decoration: underline;"><a href="index.php?p=view_link&id=<?=$urlpost['shared_url_id']?>"><?=((strlen($urlpost['url_desc']) > 35) ? substr($urlpost['url_desc'],0,35) : $urlpost['url_desc'])?><span style="display: none;"><?=$urlpost['url_desc']?></span></a></td>
                                 <td style="width:25%"><a href="index.php?p=view_link&id=<?=$urlpost['shared_url_id']?>"><?=($urlpost['email_id'] == '') ? 'Sponsored' : $urlpost['email_id']?></a></td>
                                 <td style="width: 15%; padding-left: 0px; padding-right: 0px;"><?=date('m/d/Y', $urlpost['shared_time'])?>   <?=date('h:i a', $urlpost['shared_time'])?></td>
                              </tr>
                              <?php } ?>
                              <?php
                               if($i == 1){
               ...
               <div class="text-right"><small>Page <?=isset($_GET['page']) ? $_GET['page'] : 1?> of <?=$total_page_count?></small></div>
               <?php } ?>
               <div class="bottom-nav-link table-design<?=((isset($list_shared_links_by_admin) and count($list_shared_links_by_admin)>0) ? ' margin-none' : '')?>">
