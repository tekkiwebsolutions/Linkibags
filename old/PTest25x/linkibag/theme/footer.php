	...
	<script>
		...
		function fiter_with_folder_dashboard(cid){
			if(cid==""){
				...
				if(cid=='0'){
					window.location.href="index.php?p=mylinks&cid="+cid+"&trash=1";
				}else{
					window.location.href="index.php?p=mylinks&cid="+cid;
				}
			}
		}
	</script>
...
<script>
	...
	function forget_password_submit() {
		var email = $('#signup_email_id').val();
		$('#singup_forget_link').html('Sending request...');
		$.ajax({
			type: "POST",
			url: "index.php",
			data: {email_adr: email, form_id: 'forget_password', ajax: 1},
			success: function(res) {
				res = JSON.parse(res);
				$('#singup_forget_link').html('Forgot my password');
				$('#singup_forget_link').hide();
				if(res.msg) {
