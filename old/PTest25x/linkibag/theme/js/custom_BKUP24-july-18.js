...
function add_newletter(){
	...
	$.ajax({
		...
		success: function(res2) {
			res2 = JSON.parse(res2);
			if(res2.success === true){
				$("#subscribe_email").css('color','green');
				$("#subscribe_email").val(res2.msg);
				//$("#dialog_success").html(res2.msg);
				//$("#dialog_success" ).dialog( "open" );
				//$(".ui-dialog-titlebar.ui-widget-header.ui-corner-all.ui-helper-clearfix").addClass( "dialog_success" );
			}else if(res2.success === false){
				$("#subscribe_email").css('color','red');
				$("#subscribe_email").val(res2.msg);
					//$("#dialog_error").html(res2.msg);
					//$( "#dialog_error" ).dialog( "open" );					
			}
...
function multiple_load_share_link_form(type){
	...
	if(jQuery('input:checkbox[class=urls_shared]:checked').length > 0){
		...
		if(type === 'share'){
			...
			$("#dialog_confirm").html("You are about to delete link(s)? These can not be Undo.");
	...
	if(jQuery('input:checkbox[class=urls_shared]:checked').length > 0){
		...
		if(type === 'category'){
			...
			if(unfriend === '1'){
				...
				cat = -2;
				$("#dialog_confirm").html("Selected user(s) will be deleted from friends list? it will go to spam.");
			}else{	
				$("#dialog_confirm").html("You are about to move friends to this group?");
			}	
		}
		$("#dialog_confirm").dialog({
			autoOpen: false, 
		...
		buttons: {
			"Yes" : function() {
				...
				success: function(res2){
					...
					$("#dialog_success").html('Deleted successfully.');
					$( "#dialog_success" ).dialog( "open" );
					$(".ui-dialog-titlebar.ui-widget-header.ui-corner-all.ui-helper-clearfix").addClass( "dialog_success" );
				}
			});
