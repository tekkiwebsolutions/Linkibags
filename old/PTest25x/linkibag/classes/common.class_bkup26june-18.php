...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <style type="text/css">
			...
			h1{margin: 0;}
			.big{color: #3e3e3e;font-size: 22px;margin-top: 4px;}
            ...
            <tr>
                <td>
                    <table class="content" align="center" cellpadding="0" cellspacing="0" border="0">
                        ...
                        /*
                        <tr>
                        <div style="margin: auto; width: 100%; font-family: tahoma; font-size: 16px; color: rgb(34, 34, 34); line-height: 22px; text-align: left !important" >
	<p>Dear Sir/Madam,</p>
	<p  style="text-indent: 61px; text-align: left; border-bottom: 1px solid rgb(204, 204, 204); padding: 0px 0px 17px;">
	Thank you for registering with LinkiBag Free Account. 
	</p>
	...
	Your Password is :&nbsp; &nbsp; '.$var['password'].'<br /><br /></p>
						...
						<tr>
                            <td>
								...
								<p class="big">You’re almost done. Click on link below to confirm to finish creating your LinkiBag account.</p>
								<a class="btn" href="'.$var['verified_link'].'">Confirm Account</a>
                            </td>
                        </tr>
						<tr>
                            <td>
                                <p>This message was send to '.$var['email_id'].'. if you have questions or complaints, please <a href="'.$this->get_bit_ly_link('http://www.linkibag.net/PTest25x/linkibag/index.php?p=contact-us').'"><b>contact us.</b></a> We’re here to help.</p>
                            </td>
                        </tr>
						<tr>
                            <td>
                                <p><a href="'.$this->get_bit_ly_link('http://www.linkibag.net/PTest25x/linkibag/index.php?p=terms-of-use').'">Terms of Use</a> &nbsp; | &nbsp; <a href="'.$this->get_bit_ly_link('http://www.linkibag.net/PTest25x/linkibag/index.php?p=terms-of-use').'">Privacy Policy</a></p>
                            </td>
                        </tr>
                    </table>
                </td>
	...
	function get_paging_link($total_pages, $this_page){
		...
		if($show_backto_button == true){
			$backto_button_link = '<a class="btn btn-default dark-gray-bg" style="margin-right:2px;" href="'.$backtopage.'"'.(($backtopage == '') ? ' disabled' : '').'>Back to page</a>';
		}else{
			$backto_button_link = '';
		}
	...
	function edit_groups_and_cat($uid, $id, $type, $mode, $page){
		if($mode == 'edit'){
			...
			if($type == 'group')
				...
				$sql = "SELECT p.first_name,us.shared_time,us.shared_url_id,us.url_msg,ur.url_id,ur.url_title,ur.url_value,ur.url_desc,u.email_id FROM `user_urls` ur, users u, user_shared_urls us, profile p WHERE p.uid=u.uid and us.uid=u.uid and ur.url_id=us.url_id and us.url_cat=ur.url_cat and us.shared_url_id=:id and us.uid=:uid";
			...
			if(isset($result['group_id']) and $result['group_id'] > 0){
				...
				if($type == 'web_resource_list_links_notes'){
					...
					$form_array[] = array('label'=>'Message','name'=>'url_msg','val'=>$url_msg);
				}else{
	...
	function get_share_link_popup($url_id){	
		...
		$share_link_body = '		
			...
			function users_count_url($uid){	
				...
				$sql = "SELECT COUNT(friend_id) as total_pending_invites FROM `user_friends` WHERE fid=:id and status='0' and read_status='0'";	
			...
			function users_count_shared_url($uid){
				...
				if(isset($_GET['cid']))
				 ...
				 $sql = "SELECT COUNT(us.shared_url_id) as total_shared_urls FROM `user_urls` ur, users u, user_shared_urls us WHERE us.shared_to=u.uid and ur.url_id=us.url_id and us.shared_to=:id and us.url_cat=:cid and ur.status='1' and read_status='0'";
