...
class commonClass extends DB
{
	...
	function adminlogin($username, $pwd, $remember=0)
	{
		...
		$pwd = md5($pwd);		
			...
			$sqlAdver = "SELECT * FROM `admin_advertisement` WHERE `username`= :user and `password` = :pass LIMIT 1";
		//$pwd = md5($pwd);		
		$rowAdver= $this->row($sqlAdver, array("user"=>$username, "pass"=>$pwd));
	
		if(isset($rowAdver['adid']) and $rowAdver['adid']>0)
		{
			if($remember==1){
				setcookie('advertiser_uid', $rowAdver['adid'], strtotime('+1 days'));
				setcookie('advertiser_website', 'Linkibag advertiser', strtotime('+1 days'));
				setcookie('advertiser_logged_in', 'Yes', strtotime('+1 days'));
			}else{
				$_SESSION['advertiser_uid'] = $rowAdver['adid'];
				$_SESSION['advertiser_website'] = 'Linkibag advertiser';
				$_SESSION['advertiser_logged_in'] = 'Yes';
			}
			return true;
		}else
		{
...
Team '.SITE_NAME;
	...
	{
		if(isset($_COOKIE['advertiser_uid']) && isset($_COOKIE['advertiser_website']) && $_COOKIE['advertiser_website']=="Linkibag advertiser")
		{
			return true;
		}
	...
	function getcurrent_admin()
	{
		...
		{
			$sql = "SELECT * FROM `admin_advertisement` WHERE adid= :user";
			$row= $this->row($sql, array("user"=>$_COOKIE['advertiser_uid']));
			return $row;
		}
		elseif(isset($_SESSION['advertiser_uid']) && isset($_SESSION['advertiser_website']) && $_SESSION['advertiser_website']=="Linkibag advertiser")
		{			
			$sql = "SELECT * FROM `admin_advertisement` WHERE adid= :user";
			$row= $this->row($sql, array("user"=>$_SESSION['advertiser_uid']));
			return $row;
		}
		else
		{
