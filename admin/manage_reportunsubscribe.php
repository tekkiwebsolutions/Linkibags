<?php
$breadcrumb = 'Unsubscribe Management';
$title = '<i class="fa fa-table"></i> Manage Unsubscribe reports';
$item_per_page = 30;

if(isset($_GET['changeid']) and isset($_GET['status'])) {
	$co->query_update('unsubscribe', array('status'=>$_GET['status']), array('us_id'=>$_GET['changeid']), 'us_id=:us_id');
	echo '<script type="text/javascript">window.location.href="main.php?p=manage_reportunsubscribe"</script>';
	exit();
}

$sql = "SELECT * from unsubscribe WHERE us_id>0";

$cond_arr = array();
if(isset($_GET['save'])){
	if(isset($_GET['name_search']) and $_GET['name_search']!=''){
		$sql .= " AND mail_id LIKE :key";
		$cond_arr['key'] = "%".$_GET['name_search']."%";
	}
}
$sql .= " ORDER BY us_id DESC";

$this_page = "p=manage_reportunsubscribe";				
...
<div>    
    <table class="table table-hover table-striped">
		<thead>
			<tr>
				<th>Id</th><th>Email</th><th>Unsubscribe On</th><th>Unsubscribe?</th><th>Actions</th>
			</tr>
		</thead>
	   <tbody>
			<?php
						...
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									...
									<h3 id="myModalLabel3"><?=($row['status']==1 ? 'Unsubscribe' : 'Subscribe')?> Email</h3>
								</div>
								...
								<div class="modal-footer">
									<div class="btn-group">
										...
										<a href="main.php?p=manage_reportunsubscribe&amp;changeid=<?=$row['id']?>&status=<?=($row['status']==1 ? 0 : 1)?>"><button class="btn btn-info"><span class="glyphicon glyphicon-right"></span> Confirm</button></a>
									</div>
								</div>
							</div><!-- /.modal-content -->
						</div><!-- /.modal-dalog -->
