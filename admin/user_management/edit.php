...
if (isset($_POST['save'])){ 
    ...
    */
    ...
    if ($success == true){
        ...
        $up['verified'] = $_POST['verified'];
        $up['verified_time'] = time();
        $up['updated'] = time();
        $dd = $co->query_update('users', $up, array('id' => $_POST['id']) , 'uid=:id');		 
        unset($up);
        ...
        $dd =$co->query_update('profile', $up, array( 'id' => $_POST['id'] ) , 'uid=:id');
...
?>
	...
	<form method="post" enctype="multipart/form-data" class="form-horizontal">		
			...
			<div class="col-sm-8">
				<select class="form-control linkibox_select" name="salutation">
					...
					foreach ($salutation as $sal_val => $sal_name){
						$sel = '';
						if (isset($row['salutation']) and $row['salutation'] == $sal_val) $sel = ' selected="selected"';
				...
				<select class="form-control" name="country" onchange="show_states(this.value)">
					...
					<?php
						foreach ($countries as $country) {
							$sel = '';
							if (isset($row['country']) and $row['country'] == $country['id']) $sel = ' selected="selected"';
				...
				<div class="col-sm-4">	
					<select name="state" class="form-control">
						...
						<?php
							...
							{
								$sel = '';
								if (isset($row['state']) and $row['state'] == $state['id']) $sel = ' selected="selected"';
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
				<input type="radio" name="verified"  value="1" checked /> Yes
				<input type="radio" name="verified"  value="0" <?php if ($row['verified'] == 0){echo 'checked';} ?> /> No
			</div>
		</div>
		...
		<div class="form-group">		
			...
			<div class="col-sm-4 col-sm-offset-2">
				<a class="btn btn-xs btn-danger del_links" href="main.php?p=user_management/delete_info&amp;delid[]=<?=$row['uid'] ?>"><i class="fa fa-fw fa-trash"></i>Delete User</a>		
			</div>
		</div>
	</form>
