...
$title = '<i class="fa fa-table"></i> Manage Public Category';
$item_per_page = 12;

$sql = "SELECT c.*,p.first_name,p.last_name from user_public_category c LEFT JOIN profile p ON p.uid=c.uid WHERE c.uid>='0'";
$cond_arr = array();
if(isset($_GET['save'])){
...
$this_page = "p=category_management/manage_public_category";				
...
?>
								...
								<form class="form-horizontal" enctype="multipart/form-data" method="GET">									
									<input type="hidden" value="category_management/manage_public_category" name="p">									
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover table-striped">
										<thead>
											<tr>
												<th>Sr no.</th><th>Category</th><th>Name</th><th>Status<th>Created On</th><th>Actions</th>
											</tr>
										</thead>
									   <tbody>
											...
											<tr>
												<td><?=$i?></td>
												<td><?=$row['cname']?></td>
												<td><?=((isset($row['first_name']) and $row['first_name'] != '' )? $row['first_name'] : 'Admin')?> <?=((isset($row['first_name']) and $row['last_name'] != '' )? $row['last_name'] : '')?></td>
												<td><?=$status[$row['status']]?></td>
												<td><?=date('j F, Y',$row['created_time']);?></td>                                      
												<td> 
													<a href="main.php?p=category_management/edit_public_user_cat&amp;id=<?=$row['cid']?>" class="btn btn-xs btn-primary"> <i class="fa fa-fw fa-edit"></i> Edit</a>
