			...
			if(isset($_POST['email_to_friends']) and $_POST['email_to_friends'] == 1){
				...
				if(isset($shared_allusers) and count($shared_allusers) > 0){
					foreach($shared_allusers as $shared_user){
						...
						$show_email_list_in_msg .= '<h4 class="text-gray">'.$to.'</h4>';
				...
				$msg = '<div class="sharing-links-success-panel">							
		<div class="sharing-panel-body">
			<h4>Success! Your link was emailed to the following users:</h4><br>
			'.$show_email_list_in_msg.'
			<br>
			<h4>Your link also available to web users via Share ID '.$_SESSION['share_number'].' during next 30 minutes.</h4>
						...
						foreach($groups as $group_info){
							...
							$new_row .= '
								<tr class="'.$class_name.'" id="record_'.$group_info['group_id'].'">
									<td style="width:50%"><span><input type="checkbox" class="grouping" value="'.$group_info['group_id'].'" name="group_name[]"></span> &nbsp; <a href="#">'.$group_info['group_name'].'</a> <span>&nbsp; <a href="#" onclick="load_edit_frm('.$group_info['group_id'].', \'group\')"><i class="fa fa-pencil"></i></a></span></td>
									<td style="width:20%"><a class="btn btn-sm" heaf="#" onclick="error_dialogues(\'There is no friends in this group\')">Empty</a></td>
	...
	if(isset($_POST['form_id']) and $_POST['form_id']=="newsletter_us"){
		...
		if($_POST['email_id']==""){
			...
			if(filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
				...
				$msg .= "Email address is already exist, try another <br/>";
				$success=false;
			}
		}	
		...
		if($success==true){
			...
			$msg .= "Success. Thank you for joining! <br/>";	
		}
