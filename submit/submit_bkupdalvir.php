...
if($_SERVER['REQUEST_METHOD']=='POST'){
	if(isset($_POST['form_id']) and $_POST['form_id']=="register"){								
		...
		*/
		...
		if(isset($_POST['email_id']) and $_POST['email_id']!=''){
			...
			if(filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
				$msg .= "$email is not a valid email address <br/>";
				$success=false;
			}elseif(!in_array($domain_name, $domain_name_array)){
				$msg .= "LinkiBag did not recognize your email vendor as a public email provider from our list <br/>";
				$success=false;
		...
		*/
		...
		if($success==true){			
			...
			$msg .= "Congratulations! You are almost done. To complete registration please click on the link that we just sent to your email address. Thank you again.";
		...
		if($url==""){
			...
			if(!preg_match($pattern_1, $url)){			
				...
				if(strpos($url_headers[0],'200')===false) {
				    $co->setmessage("error", "This webpage is not available. Please check it and try again");
					$success=false;
				}else{
					//virus total
					$ch = curl_init();
						...
						<div class="web-resources-list-links-single-body">
						  <div class="col-md-12">
							<div class="row web-resources-list-row-2">
								...
								<div class="col-md-4">
									<p class="text-blue"><a href="#" onclick="load_edit_frm('.$cat_info['shared_url_id'].', \'web_resource_list_links\')"><i class="fa fa-pencil"></i></a> '.$cat_info['url_desc'].'</p>
								</div>
								<div class="col-md-3">
			...
			if((isset($_POST['comment']) and $_POST['comment'] != '') OR (isset($_POST['notes']) and $_POST['notes'] != '')){
				...
				if(isset($_POST['id']) and $_POST['id'] > 0){
					...
					$new_row = '
						...
						$new_row .=			
									'<a href="#" onclick="load_edit_frm('.$cat_info['shared_url_id'].', \'web_resource_list_links_comment\')"><i class="fa fa-pencil"></i></a> ';
								}	
						...
						$new_row .=	'</p>
								...
								if(isset($current['uid']) and $current['uid'] == $cat_info['uid']){			
								$new_row .=			
									'<a href="#" onclick="load_edit_frm('.$cat_info['shared_url_id'].', \'web_resource_list_links_notes\')"><i class="fa fa-pencil"></i></a> ';
								}	
	...
	if(isset($_POST['form_id']) and $_POST['form_id']=="send_friend_request"){		
		...
		if($success==true){				
			foreach($emails_ids as $email_ids){
				...
				$up['request_to'] = ($uid > 0) ? $uid : $email_ids;
	...
	if(isset($_POST['form_id']) and $_POST['form_id']=="share_links"){
		...
		if(isset($_POST['url_msg_edit_for_share_links']) and $_POST['url_msg_edit_for_share_links'] == 1){
			if(isset($_POST['id']) and $_POST['id'] > 0){
				$urlpost = $co->query_first("SELECT p.first_name,us.shared_time,us.shared_url_id,us.url_msg,ur.url_id,ur.url_title,ur.url_value,ur.url_desc,u.email_id FROM `user_urls` ur, users u, user_shared_urls us, profile p WHERE p.uid=u.uid and us.uid=u.uid and ur.url_id=us.url_id and us.url_cat=ur.url_cat and us.shared_url_id=:id and us.uid=:uid",array('uid'=>$current['uid'],'id'=>$_POST['id']));
					if(isset($urlpost['shared_url_id']) and $urlpost['shared_url_id'] > 0){
						$up_val = array();
						$up_val['url_msg'] = $_POST['url_msg'];
						$up_val['url_msg_time'] = time();				
						$co->query_update('user_shared_urls', $up_val, array('id'=>$_POST['id'], 'uid'=>$current['uid']), 'shared_url_id=:id and uid=:uid');
						unset($up_val);	
						$urlpost = $co->query_first("SELECT p.first_name,us.shared_time,us.shared_url_id,us.url_msg,ur.url_id,ur.url_title,ur.url_value,ur.url_desc,u.email_id FROM `user_urls` ur, users u, user_shared_urls us, profile p WHERE p.uid=u.uid and us.uid=u.uid and ur.url_id=us.url_id and us.url_cat=ur.url_cat and us.shared_url_id=:id and us.uid=:uid",array('uid'=>$current['uid'],'id'=>$_POST['id']));
						if($urlpost['url_msg'] != '')
						...
						$new_row = '
							...
							<td style="width:10%"><a href="#" onclick="load_edit_frm('.$urlpost['shared_url_id'].', \'add_sharing_link_url_msg\');"><i class="fa fa-pencil" aria-hidden="true"></i></a>	
							</td>
							';	
				...
				foreach($_POST['urls'] as $urls_id){
					if(isset($shared_users) and count($shared_users) > 0){
						foreach($shared_users as $shared_user){
							...
							$url_info = $co->query_first("select us.url_msg,us.url_msg_time,ur.url_id,ur.url_cat,ur.url_value,ur.url_desc from user_urls ur, user_shared_urls us where us.url_id=ur.url_id and us.shared_url_id=:urls",array('urls'=>$urls_id));
					...
					$_SESSION['urls_for_print_pdf'] .= 'url[]='.$urls_id.'&';				
				}
	...
	*/
	...
	<h4>Share Link(s) Report</h4>

<div style="overflow: hidden; margin: 13px 0px;">
  <div style="float: left; width: 30%;"><h4>Link:</h4></div>
  <div style="float: right; width: 70%;"><h4>'.$show_url_val_list_in_msg.'</h4></div>
</div>
<div style="overflow: hidden; margin: 13px 0px;">
  <div style="float: left; width: 30%;"><h4>Message:</h4></div>
  <div style="float: right; width: 70%;"><h4>'.$show_url_desc_in_msg.'</h4></div>
</div><div style="overflow: hidden; margin: 13px 0px;">
  <div style="float: left; width: 30%;"><h4>Shared with:</h4></div>
  <div style="float: right; width: 70%;"><h4>'.$show_email_list_in_msg.'</h4></div>
</div>';
if(!(isset($_POST['disable_share_id']) and $_POST['disable_share_id'] == 1)){
...
$msg .= '
<div style="overflow: hidden; margin: 13px 0px;">
  <div style="float: left; width: 30%;"><h4>Share ID Access:</h4></div>
  <div style="float: right; width: 70%;"><h4>'.date("m/d/Y h:ia T", $tim).'</h4></div>
  </div>

  <h6>
			<button type="button" id="print_link" onclick="print_pdf_urls()" class="btn orang-bg">Save</button>
			<button type="button" id="print_links" onclick="printPageArea(\'all_records\')" class="btn orang-bg">Print</button>
			<button type="button" class="btn orang-bg" role="button" aria-disabled="false" title="close" style="display: none;">Close</button>
			</h6></div></div>
		...
		if($success == true){
			...
			if(isset($_POST['group_name']) and $_POST['group_name'] != ''){	
				...
				if(isset($_POST['id']) and $_POST['id'] > 0){
					...
					$new_row = '
						<td style="width:50%"><span><input type="checkbox" class="grouping" value="'.$group_info['group_id'].'" name="groups[]"></span> &nbsp; <a href="#">'.$group_info['group_name'].'</a> <span>&nbsp; <a href="#" onclick="load_edit_frm('.$group_info['group_id'].', \'group\')"><i class="fa fa-pencil"></i></a></span></td>
						...
						foreach($groups as $group_info){
							...
							$new_row .= '
								<tr class="'.$class_name.'" id="record_'.$group_info['group_id'].'">
									<td style="width:50%"><span><input type="checkbox" class="grouping" value="'.$group_info['group_id'].'" name="groups[]"></span> &nbsp; <a href="#">'.$group_info['group_name'].'</a> <span>&nbsp; <a href="#" onclick="load_edit_frm('.$group_info['group_id'].', \'group\')"><i class="fa fa-pencil"></i></a></span></td>
									<td style="width:20%"><a class="btn btn-sm" href="#" onclick="error_dialogues(\'There is no friends in this group\')">Empty</a></td>
				...
				if(isset($records) and count($records) > 0){
					...
					foreach($records as $list){
						...
						$new_row .= '<tr class="'.$class_name.'" id="record_'.$list['group_id'].'">
							<td style="width:50%"><span><input type="checkbox" class="grouping" value="'.$list['group_id'].'" name="groups[]"></span> &nbsp; <a href="#">'.$list['group_name'].'</a> <span>&nbsp; <a href="#" onclick="load_edit_frm('.$list['group_id'].', \'group\')"><i class="fa fa-pencil"></i></a></span></td>
							<td style="width:20%"><a class="btn btn-sm" href="#" onclick="'.$empty_link.'">Empty</a></td>
							<td class="text-center" style="width:15%">'.$group_info['confirmed'].'</td>
							<td class="text-center" style="width:15%">'.$group_info['total_friend'].'</td>
							</tr>
		...
		if($success == true){
			if(isset($_POST['share_url']) and count($_POST['share_url']) > 0){					
				...
				*/
				...
				echo json_encode(array('msg'=>$msg,'new_row'=>$new_row,'msg_content'=>$msg_content,'share_url'=>$_POST['share_url'], 'total_unread'=>$total_friends));
			}		
			exit();	
		}else{
